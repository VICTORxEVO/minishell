...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/12/22 11:48:55 by ysbai-jo          #+#    #+#             */
/*   Updated: 2024/12/22 12:05:27 by ysbai-jo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static bool	hd_handleline(char *line, char *delimit, int fd)
{
	if (!line)
	{
		printf(PRGM_NAME HD_WARNING, delimit);
		return (1);
	}
...
int	hd(char *delimit)
{
	...
	if (unlink(tmpfile) < 0)
		ft_putstr_fd(PRGM_NAME HD_TMP_FAIL, 2);
	return (fd);
}
