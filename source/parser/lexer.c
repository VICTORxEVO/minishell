...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/12/22 12:07:11 by ysbai-jo          #+#    #+#             */
/*   Updated: 2024/12/22 12:08:50 by ysbai-jo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*lexer_get_word(char *line, long long *i)
{
	...
	while (line[*i] && !ft_isspace(line[*i], NULL) && !istoken(line[*i],
			ALL_TKN))
	{
		...
		if (!line[*i] || ft_isspace(line[*i], NULL) || istoken(line[*i],
				ALL_TKN))
			break ;
		(*i)++;
	}
