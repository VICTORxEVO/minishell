...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/12/22 11:49:19 by ysbai-jo          #+#    #+#             */
/*   Updated: 2024/12/22 11:49:20 by ysbai-jo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool	prepare_ifof(t_cmd *cmd_list)
{
	...
	{
		if (lexer->type == AMBIG_RDRT)
			return (redirection_cleanup(lexer, cmd_list->ifd, cmd_list->ofd),
				1);
		if (lexer->type == IN_RDRT || lexer->type == HERE_DOC_FD)
			cmd_list->ifd = redirection(lexer->next->content, lexer->type,
					cmd_list->ifd);
		else if (lexer->type == OUT_RDRT_APP || lexer->type == OUT_RDRT_OW)
			cmd_list->ofd = redirection(lexer->next->content, lexer->type,
					cmd_list->ofd);
